...
module.exports = class Neuron {
    ...
    signal(X, Y) {
        ...
        for (let Z = 1; Z <= this.dimension; Z++){
            this.data[X][Y][0] += RELU(this.data[X-1][Y][0] * this.data[X-1][Y][Z])
        }
    ...
    train_layer(X, expected) {
        ...
        // update the weights according to the proposed signals, one layer at a time
        for (let Y = 0; Y < this.dimension; Y++){
    ...
    train(expectation){
        ...
        this.stability = 1  // need to reset this each time for clarity
